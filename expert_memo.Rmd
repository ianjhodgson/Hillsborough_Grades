...
df_schools <- school_grades %>% 
  filter(district_name == "HILLSBOROUGH", 
         informational_baseline_grade_2023 == "D/F") %>% 
...
school_grades %>% 
  ...
  summarise(mean_share_low_income = mean(percent_of_economically_disadvantaged_students, na.rm = T)/100,
            Schools = n()) %>% 
  select(type, Grade, Schools, mean_share_low_income) %>% 
  rename(Type = type,
         `Share Low-Income` = mean_share_low_income) %>% 
  arrange(Type, Grade) %>% 
...
sesir_2122_analysis %>% 
  ungroup() %>% 
  filter(type == "Middle",
         str_detect(type_of_incident, "FIGHT|ATTACK")) %>% 
  group_by(school_name) %>% 
  summarise(fights = sum(total_incidents), 
            fights_percap = sum(incidents_per_cap)) %>% 
  arrange(desc(fights_percap)) %>% 
  ungroup() %>% 
  summarise(fights = mean(fights),
            fights_percap = mean(fights_percap))
...
sesir_analysis %>% 
  ...
  group_by(school_name, Grade, type) %>% 
  summarise(total_incidents = sum(total_incidents),
            incidents_per_cap = sum(incidents_per_cap),
            leo_incidents_per_cap = sum(leo_incidents_per_cap)) %>% 
  arrange(type, desc(total_incidents)) %>% 
  group_by(Grade, type) %>% 
  summarise(Schools = n(), 
            `Average Incidents` = mean(total_incidents),
...
sesir_analysis %>% 
  ...
  summarise(Grade = first(Grade), 
            `Total Incidents` = sum(total_incidents),
            Incidents = sum(incidents_per_cap, na.rm = T)*100,
            `Incidents Reported to LEO` = sum(leo_incidents_per_cap, na.rm = T)*100) %>% 
...
### SESIR 2017-18
enrollment_1819 <- read_xlsx("data/FLDOE/Enrollment/1819MembBySchoolByGrade.xlsx",
                             sheet = 1,
                             skip = 2) %>% 
  janitor::clean_names() %>% 
  filter(district == "HILLSBOROUGH") %>% 
...
sesir_1819 <- read_xls("data/FLDOE/Discipline/sesir1819a-h.xls", 
                       sheet = 1, 
                       skip = 5) %>% 
  janitor::clean_names() %>% 
  filter(district_name == "HILLSBOROUGH") %>% 
  ...
  filter(Grade != "I") %>% 
  left_join(enrollment_1819, by = c("district_name" = "district", "school_number" = "sch_number"))
...
school_grades %>% 
  ...
  left_join(enrollment_2223 %>% 
              select(-district) %>% 
              rename(students_2223 = number_of_students),
            by = c("sch_number")) %>% 
  left_join(enrollment_1718 %>% 
              select(-district) %>% 
              rename(students_1819 = number_of_students),
            by = "sch_number") %>% 
  filter(Grade == "D/F") %>% 
  select(school_name, 
         students_2223, 
         students_1819) %>% 
  arrange(school_name) %>% 
  summarise(students_2223 = sum(students_2223),
            students_1819 = sum(students_1819))
...
sesir_1819 %>% 
  ...
  mutate(year = 2019) %>% 
  bind_rows(sesir %>%  
              select(district_number = dist_number, 
  ...
  mutate(year = 2023)) %>% 
  # filter(type_of_incident %>% str_detect("SCHOOL TOTAL"),
  #        Grade == "D/F") %>%
    filter(type_of_incident %>% str_detect("FIGHT|ATTACK|THREAT"),
           Grade == "D/F") %>%
  group_by(year) %>% 
  summarise(incidents = sum(total_incidents, na.rm = T))
...
absent %>% 
  filter(Grade == "D/F") %>% 
  select(school_name, pct_10 = percent_of_students_absent_10_percent_or_more, days_21 = percent_of_students_absent_21_or_more_days) %>% 
  arrange(desc(pct_10))
...
```{r absenteeism pre-pandemic, include = T}
absent_1819 <- read_xlsx('data/FLDOE/Absenteeism/1819ABS21Days10Comparison.xlsx', 
                    sheet = 2, 
                    skip = 2,
                    na = "*") %>% 
  janitor::clean_names() %>% 
  filter(district_name == "HILLSBOROUGH") %>% 
  select(school_number, district_number, school_name, 
         percent_of_students_absent_21_or_more_days_1819 = percent_of_students_absent_21_or_more_days,
         percent_of_students_absent_10_percent_or_more_1819 = percent_of_students_absent_10_percent_or_more)

absent %>% 
  left_join(absent_1819) %>% 
  mutate(Grade = ifelse(Grade %in% c("D", "F"),
                        "D/F", Grade),
         type = ifelse(type == "Combination", "Middle", type)) %>% 
  filter(type != "High") %>% 
  mutate(diff_21_days = percent_of_students_absent_21_or_more_days - percent_of_students_absent_21_or_more_days_1819,
         diff_10_days = percent_of_students_absent_10_percent_or_more - percent_of_students_absent_10_percent_or_more_1819) %>% 
  group_by(type, Grade) %>% 
  summarise(diff_21_days = mean(diff_21_days, na.rm = T),
            diff_10_days = mean(diff_10_days, na.rm = T),
            pct_10pct = mean(percent_of_students_absent_10_percent_or_more, na.rm = T),
            pct_21days = mean(percent_of_students_absent_21_or_more_days, na.rm = T ),
            pct_10pct_pre = mean(percent_of_students_absent_10_percent_or_more_1819, na.rm = T),
            pct_21days_pre = mean(percent_of_students_absent_21_or_more_days_1819, na.rm = T ))
```
...
hills_teachers %>% 
  ...
  summarise(ave_tenure = mean(tenure, na.rm = T)/365.25,
            ...
            share_tenure_le3yr = sum(tenure_le3yr, na.rm = T)/n(),
            share_age_le30yr = sum(age_le30yr, na.rm = T)/n()) %>% 
  mutate(ave_tenure = round(ave_tenure, 1), 
         ave_age = round(ave_age, 1)) %>% 
bind_rows(hills_teachers %>% 
            summarise(ave_tenure = mean(tenure, na.rm = T)/365.25,
                      ave_age = mean(age, na.rm = T)/365.25,
                      share_tenure_le3yr = sum(tenure_le3yr, na.rm = T)/n(),
                      share_age_le30yr = sum(age_le30yr, na.rm = T)/n()) %>% 
...
## Have test scores improved in 2023-24?

In February, the district released PM2 results for the 2023-24 school year. We can compare the share of students who score a level 3 and up in 2023-24 to the shares the year before.

Results are released by grade for math and English. I combine results for each subject across grades for each Hillsborough school. 

``` {r PM2 scores analysis, include = T}
pm2_results <- read_csv("data/FAST_2024/fast_2024_clean.csv") %>%
  left_join(school_grades %>% 
              filter(district_name == "HILLSBOROUGH") %>% 
              select(school_number, 
                     Grade = informational_baseline_grade_2023, 
                     ...
                     type),
            by = "school_number") %>% 
  filter(!is.na(Grade), Grade != "I") %>% 
  mutate(subject = str_extract(exam, "Math|ELA$"))

hills_average_change <- pm2_results %>% 
  group_by(subject, type) %>% 
  filter(!is.na(level3_plus_22_23), 
         !is.na(level3_plus_23_24)) %>%
  summarise(level3_plus_22_23 = weighted.mean(level3_plus_22_23, student_count_22_23),
            level3_plus_23_24 = weighted.mean(level3_plus_23_24, student_count_23_24)) %>% 
    mutate(level3_plus_change = (level3_plus_23_24-level3_plus_22_23)*100)

temp <- hills_average_change %>% 
              mutate(school_name = "District Average") %>% 
  bind_rows(pm2_results %>% 
  filter(Grade =="D/F") %>%
  group_by(school_name, subject, type) %>% 
  summarise(level3_plus_22_23 = weighted.mean(level3_plus_22_23, student_count_22_23, na.rm = T),
            level3_plus_23_24 = weighted.mean(level3_plus_23_24, student_count_23_24, na.rm = T)) %>% 
  mutate(level3_plus_change = (level3_plus_23_24-level3_plus_22_23)*100) %>% 
  select(school_name, type, subject, starts_with("level3_plus")) %>% 
  arrange(level3_plus_change)) %>% 
  arrange(type)
  
write_csv(temp, "data/FAST_2024/fast_2024_pm2_summary.csv", na = "")

temp %>% filter(subject == "Math", 
                type != "High") %>% 
  ungroup() %>% 
  select(school_name, everything()) %>% 
  rename(`2022-23` = level3_plus_22_23, 
         `2023-24` = level3_plus_23_24,
         `Change (percentage points)` = level3_plus_change) %>% 
  select(-subject) %>% 
  gt(groupname_col = "type") %>% 
  fmt_percent(columns = 3:4, decimals = 1) %>% 
  fmt_number(columns = 5, decimals = 1) %>% 
  sub_missing() %>% 
  tab_header(title = "Math scores fell at the majority of D/F schools",
             subtitle = "Share of students scoring level 3 and above on PM2 exams")

temp %>% filter(subject == "ELA", 
                type != "High") %>% 
  ungroup() %>% 
  select(school_name, everything()) %>% 
  rename(`2022-23` = level3_plus_22_23, 
  ...
  fmt_number(columns = 5, decimals = 1) %>% 
  sub_missing() %>% 
  tab_header(title = "English scores were mixed at D/F schools, with roughly half improving and half falling",
             subtitle = "Share of students scoring level 3 and above on PM2 exams")
```


``` {r data export for Ron, include = F}

child_poverty <- read_csv("data/child_poverty_rates.csv")

ron_data <- school_grades %>%
  filter(district_name == "HILLSBOROUGH",
         Grade == "D/F") %>% 
  select(school_number, school_name, type, grade) %>% 
  left_join(child_poverty %>% select(school_number, child_poverty_rate = pov_share), by = "school_number") %>% 
  left_join(k_readiness %>% select(school_number, share_ready_for_kindergarten), by = "school_number") %>% 
  left_join(retention_joined %>% 
              filter(current_placement == "Stay") %>% 
              mutate(share_that_leave = 1-share_diverted) %>% 
              select(school_number, share_that_leave), 
            by = "school_number") %>% 
  left_join(hills_teachers %>% 
              select(site_number, tenure_le3yr) %>% 
              group_by(site_number) %>% 
              summarise(share_tenure_le3yr = sum(tenure_le3yr)/n()),
            by = c("school_number" = "site_number")) %>% 
  mutate(school_number_num = as.numeric(school_number)) %>% 
  left_join(absent %>% select(school_number, percent_of_students_absent_21_or_more_days), 
            by = c("school_number_num" = "school_number")) %>% 
  select(-school_number_num) %>% 
  left_join(kelly_subs_clean %>% select(school_number, total_hours_taught_by_sub = total_hours)) %>% 
  write_csv("data/report_card_data_for_ron.csv")
  
## D/F schools in top districts ----
tot_enrollment_2223 <- read_xlsx("data/FLDOE/Enrollment/2223MembBySchoolByGrade.xlsx",
                             sheet = 2,
                             skip = 2) %>% 
  janitor::clean_names()

top_dist_tom <- school_grades %>% 
  select(school_name, school_number, district_name, district_number, informational_baseline_grade_2023) %>% 
  mutate(school_number_num = as.numeric(school_number),
         district_number_num = as.numeric(district_number)) %>% 
  left_join(tot_enrollment_2223, 
            by = c("school_number_num" = "school_number",
                   "district_number_num" = "district_number")) %>% 
  mutate(flag_df = informational_baseline_grade_2023 %in% c("D", "F"),
         students = as.numeric(number_of_students), 
         students_df = students * flag_df) %>% 
  group_by(district_name, district_number) %>% 
  summarise(total_schools = n(),
            df_schools = sum(flag_df),
            total_students = sum(students, na.rm = T),
            df_students = sum(students_df, na.rm = T)) %>% 
  mutate(share_studets_in_df = df_students/total_students) %>% 
  arrange(desc(total_students)) %>% 
  write_csv("data/charts/df_schools_by_district.csv")
...
On average, state and local spending is roughly equal to or higher at lower-performing schools than higher-achieving school. When federal dollars are included, combined spending was roughly \$1,000 higher at at D/F middle schools than A schools and \$2,700 higher at D/F elementary schools.

``` {r spending, include = T}
spending <- read_xlsx("data/FLDOE/ESSA22School-PerPupil-Expend.xlsx",
                      skip = 4) %>% 
  janitor::clean_names() %>% 
    select(-school_name) %>%
  full_join(school_grades)
...
spending %>%
  filter(district_name == "HILLSBOROUGH", 
         ...
         type != "High") %>% 
  group_by(Grade, type) %>% 
  summarise(max = max(state_and_local_funds_5, na.rm = T),
            min = min(state_and_local_funds_5, na.rm = T),
            mean_sl = mean(state_and_local_funds_5, na.rm = T),
            mean_slf = mean(state_local_and_federal_funds_7, na.rm = T)) %>% 
  arrange(type, Grade)
...
school_grades %>% 
  filter(type == "Elementary",
         ...
         Grade == "D/F") %>%  
  mutate(number_failed_ese = number_of_students*english_language_arts_achievement/100,
         number_failed_math = number_of_students_math*mathematics_achievement/100) %>% 
  summarise(failed_math = sum(number_failed_math, na.rm = T)/sum(number_of_students_math, na.rm = T),
            failed_engligh = sum(number_failed_ese, na.rm = T)/sum(number_of_students, na.rm = T))
  

```


### Data export for graphics

Data for report card graphics, which include: 

* Number of students

* Share of inexperienced teachers

* Total hours taught by sub

* Students who leave

* Childhood poverty rate

``` {r data for graphics, include = F}
scorecard_export <- school_grades %>% 
  filter(district_name == "HILLSBOROUGH", 
         Grade == "D/F") %>% 
  select(school_number, 
         school_name, 
